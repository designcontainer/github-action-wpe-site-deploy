...
# Deploy via SSH
rsync -aHAXxv --inplace --numeric-ids --progress -e "ssh -v -p 22 -i ${WPE_SSHG_KEY_PRIVATE_PATH} -T -o Compression=no -o strictHostKeyChecking=no -x" --out-format="%n" --exclude=".*" $SRC_PATH "$WPE_DESTINATION"
